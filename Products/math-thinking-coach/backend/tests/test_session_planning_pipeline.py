...
from app.services import attempt_service, session_planning_pipeline

# Real content: rational-numbers has 5 questions (3 Easy, 2 Medium, 0 Hard),
# one topic. linear-equations has 44 questions (14/16/14), one topic.
...
def test_explicit_single_difficulty_still_backfills_from_other_tiers_when_empty() -> None:
    # rational-numbers has zero Hard questions. Constraint Resolver degrades
    # uniformly regardless of whether the original request was Mixed or a
    # specific tier (see constraint_resolver.py's module docstring) - the
    # student still gets a full session, just not all-Hard.
    request = AssessmentRequest(
...
def test_revision_mode_scopes_to_weak_topics_from_real_attempt_history() -> None:
    for is_correct in [False, False, False]:
        attempt_service.record_attempt(
            ...
            topic_id="topic-rational-numbers-basics", difficulty="Easy",
    ...
    assert plan.weakConceptTopicIds == ["topic-rational-numbers-basics"]
...
def test_recent_questions_are_excluded_from_a_new_practice_session() -> None:
    attempt_service.record_attempt(
        student_id="student-1", question_id="q1-rational-numbers", chapter_id="rational-numbers",
        topic_id="topic-rational-numbers-basics", difficulty="Easy", is_correct=True, attempt_number=1,
    )
    ...
    assert "q1-rational-numbers" not in selected_ids
