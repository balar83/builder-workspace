...
Success criteria met — see `ProductArchitecture.md` §12: a Class 8 student can select a chapter, attempt a question, receive guided hints, and reveal the full solution only when needed.

---

## Near-term

**Feature 015 — Shadow Mode AI Evaluation** (recommended next, not yet approved)
Run the Feature 014 AI evaluator alongside the rule-based evaluator on real traffic; log and compare; change no live behavior. Dependency: none blocking implementation, but needs product-direction input on where comparison data is stored (the product has no persistence layer today) and on confidence thresholds.

---

## Medium-term

Depends on data Shadow Mode would produce, or on persistence that doesn't exist yet:
...
*Carried forward from `ProductArchitecture.md` §10, with rationale and dependencies added.*
