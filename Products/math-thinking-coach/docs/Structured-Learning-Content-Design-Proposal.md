...
**Status:** **Slices A1 and A2 are implemented, tested, and formally CLOSED; A2b (migrating every remaining Topic-bearing chapter) is also complete.** Sections A–V are the original design/review record, kept as-is for history — **the "A2 and A3 are approved in shape only; neither is scheduled or started" line that used to appear here was stale and has been corrected** (found and fixed 2026-09-02, during a documentation audit that also caught the same stale claim repeated in `Phase-1-Handoff.md`, `PROJECT_STATUS.md`, `Backlog.md`, and `ProductArchitecture.md`). See §W.7/§W.8 for the closure record of what actually shipped and when. §M's original single-shot Slice A1 was corrected mid-implementation into an explicitly migration-state-aware design (legacy vs. structured Topics coexist during A1/A2/A3) — see §W. **Only Slice A3** (removing the now-redundant legacy `explanation`/`workedExampleContent`/`learningObjectives` fields) remains undone.
...
Confirmed in the live runtime data at closure: 4 concepts, 4 worked examples, 11 learning objectives, 40 questions all carrying exactly one `objectiveIds` entry and zero legacy `objective` fields. Legacy `.explanation`/`.workedExampleContent`/`.learningObjectives` remain populated and byte-identical in content to their pre-migration values. **At A1's close, the other 4 Topic-bearing chapters were confirmed untouched** (`concepts`/`workedExamples` empty, `objective: N` intact, fully re-exportable); **this is no longer current — see §W.8, all 4 have since migrated.** Practical Geometry remains untouched (no Topic), unaffected by any of A1/A2/A2b.

### W.7 A2 status

**A2 implemented and closed (2026-08-19).** `TopicPage.tsx`/`types/topic.ts` now render the structured `concepts`/`workedExamples` fields for any migrated chapter, with the pre-A1 legacy rendering kept as an explicit fallback for any Topic that has zero concepts — a deliberate, confirmed deviation from this section's original "legacy-field rendering logic is deleted" wording (written before A1 had shipped and the migration-window reality was fully concrete). At the time this entry was written, A Square and A Cube was the only migrated chapter; see `Development-Journal.md`'s 2026-08-19 (A2) entry for the full record of that state. **Superseded by §W.8**: every other Topic-bearing chapter has since migrated too, so the legacy-rendering fallback this paragraph describes is no longer reached by any chapter in production.

### W.8 A2b status and current closure state (added 2026-09-02)

**A2b implemented and closed.** The remaining 4 Topic-bearing chapters named as "not-yet-migrated" throughout §§W.1–W.7 — Linear Equations, Data Handling, Understanding Quadrilaterals, Rational Numbers — have all migrated onto the structured `Topic.concepts`/`.workedExamples` shape, across two commits: `51a05fe` (Linear Equations, Data Handling, Understanding Quadrilaterals, bundled with a documentation reconciliation pass) and `d7890cc` ("A2b-4", Rational Numbers, the final chapter). Neither commit is dated precisely in this document's own record — both landed sometime between the Squares & Cubes content import (`a071335`, 2026-08-19) and the Exponents and Powers content expansion (`112ace7`, 2026-09-02) — worth tightening if `Development-Journal.md` or the commit timestamps are consulted directly.

**Confirmed 2026-09-02 by reading `backend/app/data/topics.json` directly** (not asserted from this document's prior claims, which were themselves stale until this update): all 6 Topic-bearing chapters now carry structured `concepts` — Squares and Cubes (4, the A1 pilot), Rational Numbers (5), Linear Equations (3), Data Handling (3), Understanding Quadrilaterals (4) — joined by a 7th, **Exponents and Powers** (4 concepts), a chapter that didn't exist at A1's close and was presumably authored directly in the structured shape via `112ace7` rather than migrated from a legacy one; that chapter's authoring history isn't covered by this document and would need its own confirmation if a legacy `Exponents and Powers` Topic ever existed and was migrated versus born structured.

**A3 status: still not started.** A2b's completion satisfies the second of A3's two preconditions (§W.4: "every Topic-bearing chapter has migrated"); the first (A2 live-verified) was already satisfied at A2's close (§W.7). **A3 is therefore now unblocked, but nobody has picked it up** — the legacy `.explanation`/`.workedExampleContent`/`.learningObjectives` fields are still present in the schema and (per §W.6) still populated byte-identical to pre-migration content for every chapter, unused dead weight now that every chapter renders from `concepts` instead. Not scheduled or authorized by this note; flagged here so a future session finds this instead of re-deriving it.
