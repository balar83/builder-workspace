...
**A known, permanent limitation — partially addressed by Slice A1 (2026-08-17), fully resolved only after Slice A2:** the export pipeline's `transformTopic` used to join each authored section's `body` text while dropping the section `title`, leaving the runtime `Topic.explanation` field one opaque string. Slice A1 (Structured Learning Content Foundation — see `Structured-Learning-Content-Design-Proposal.md` §W for the closure record) added a structured `Topic.concepts: list[Concept]` (each with a real `title`) **additively**, alongside the still-unchanged legacy `explanation`/`workedExampleContent`/`learningObjectives` strings — both shapes are populated side by side for any migrated chapter. Only **A Square and A Cube** has been migrated so far (the Slice A1 pilot); the other four Topic-bearing chapters (Linear Equations, Data Handling, Understanding Quadrilaterals, Rational Numbers) remain on the legacy-only shape and are unaffected. `TopicPage.tsx` has **not** been updated to consume the new structured fields yet — that is Slice A2, not authorized or started. Until A2 ships, the frontend still recovers only *paragraph* structure by splitting on blank lines, even for the migrated pilot chapter; real section headings are not yet visible to a student anywhere. Slice A3 (removing the legacy fields once every chapter is migrated) is also not started.
...
4. **Topic explanation loses section headings on the Learn page** — the runtime schema/pipeline side is now done (Slice A1, `Topic.concepts` with real titles, additive, one chapter migrated — see §8). What remains is Slice A2: `TopicPage.tsx`/`types/topic.ts` need to actually consume `topic.concepts` instead of the legacy joined string, and the other 4 Topic-bearing chapters need their own migration pass. Not started; needs its own authorization.
...
| Latest commit live | `a071335` (Content import — Squares and Cubes test questions — pushed and deployed 2026-08-19, following M2.1–M2.4's `e41670a`/`87e8414`/`e120a1d`/`2e0205d`, all pushed and deployed 2026-08-17) |
| Backend health | confirmed healthy at M2.4/content-import close (`/api/v1/health`); all 6 chapters confirmed live via direct API calls with question counts 44/42/40/35/40/52 across linear-equations/data-handling/understanding-quadrilaterals/practical-geometry/rational-numbers/squares-and-cubes = **253 total** (was 241 at Slice A1's close — Squares and Cubes grew 40→52 via the `a071335` content import, all other chapters unchanged); Linear Equations confirmed serving `questionType`/`responseSpecification` for its 3 single_choice/2 multi_choice/28 numeric questions, 11 remaining `short_text`; A Square and A Cube's topic still confirmed serving structured `concepts`/`workedExamples` (4/4), unaffected by M2 — see M2.4's Development-Journal entry for the full evaluator-provenance/no-answer-leakage verification |
| Frontend health | confirmed at M2.4's close — chapter list shows all 6 chapters; Linear Equations' converted questions render as radio groups (`single_choice`) and checkbox groups (`multi_choice`) in both the anonymous and Learning Session flows, unconverted `short_text` questions unaffected; A Square and A Cube's Learn page unchanged since Slice A1 (legacy paragraph-splitting, no section headings — expected, A2 not started) |
| Known live issue at time of writing | none found during M2.4/content-import verification |
...
2. Re-run backend `pytest`, frontend `vitest run`, and the Stage 10 pipeline suite fresh — confirm 267/134/88 still holds (267 backend, not 219: M2.1–M2.4 added 48 more; 134 frontend, not 112: M2.2/M2.3 added 22 more; 88 is the Stage 10 pipeline count, not previously tracked in this section).
3. Confirm the Squares & Cubes content import (`a071335`) is actually live in production, per §18.
4. Then ask what they want to work on. **Slice A1, M2.1–M2.4, and the Squares & Cubes content import are all formally closed** — full records in `Structured-Learning-Content-Design-Proposal.md` §W, `Question-Response-Semantics-Design-Proposal.md`, and `Development-Journal.md`'s 2026-08-17/2026-08-19 entries respectively. The agreed next-priority sequence (see `Backlog.md`'s "Recommended Next"): this Documentation & Architecture Reconciliation, then Slice A2 (frontend cutover to the new structured Topic fields), then A2b (migrating the remaining 4 Topic-bearing chapters), then telemetry enrichment, then a product-fork decision, then either deterministic misconception coaching or teacher/classroom adoption, then M3 `multi_part` — **none of items after this reconciliation are scheduled or authorized to start** without an explicit go-ahead. §14's deferred-item order is still a fallback menu if none of the above is specified.
