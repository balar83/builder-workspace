...
**Release preparation — Self-Serve continuation, M3 multi-part questions, D1 concept performance, telemetry/mastery (2026-09-16, staged for release, not yet committed at the time this entry was written).** Four workstreams: (1) Self-Serve continuation — lazy learner-identity establishment on `QuestionPage.tsx`, a second "Start a Tracked Practice Session" entry point on `TopicPage.tsx`, a solution-reveal dead-end fix, and a `credentials: 'include'` fix without which the identity cookie was never sent; (2) **M3** — new `multi_part` question type, exactly `le-q25`/`le-q37`/`le-q40` retyped from `short_text` (each `maxScore` 1→2), per-part evaluation requiring every part correct for overall correctness, new `MultiPartInput` frontend component; (3) **D1** — new per-concept performance rollup (`concept_performance_service.py`, `GET /performance/me/concepts`, Dashboard/`ChapterPerformanceCard` breakdown); (4) **Telemetry/mastery** — real `hintsUsed` now reaches `attempts.hints_used` (previously always 0), new write-only `attempts.submitted_option_id` column, and the pre-existing `mastered = streak >= 3` rule's `hints_used == 0` clause is now live: **a correct answer given after a hint no longer advances the mastery streak; a correct answer without hints still does — this is an explicit, present-dated product decision, not inherited from any earlier document.** M3 and D1 had no prior authorization record anywhere in this repository before this release decision; see `Development-Journal.md`'s 2026-09-16 entry for the full account and `Question-Response-Semantics-Design-Proposal.md`'s closing note on M3 specifically. Test counts and manual-validation record: see "Engineering Health" and "Last Verified" below.

**Self-Serve Learning Loop V1, Slices 1–6b + hardening (2026-09-03 to 2026-09-09, complete, commits `2335c8a`…`8187e98`, 9 commits).** Self-serve (non-class-connected) learner identity, Progress Hub V1 (daily activity), Revision discoverability, attempt provenance tagging, Wrong-Answer Review, Recovery/Improvement Metrics, Runtime Remediation (surfacing `commonWrongAnswer`/`why`/`remediationHint`), both metrics and mistake-review surfaced on the Dashboard, and a `RequireStudent`/auth hardening pass. **This entry itself was backfilled 2026-09-16** — none of these 9 commits had a `PROJECT_STATUS.md`/`Development-Journal.md` entry until this release-preparation pass found the gap. See `Development-Journal.md`'s 2026-09-16 backfill entry for the per-commit breakdown.

**Chapter lesson-page UX slice (2026-09-02, complete, commit `44b98b7`).** TopicPage: top-of-page "Start Practice" shortcut CTA (alongside the unchanged end-of-lesson one) + progressive-disclosure worked examples (native `<details>/<summary>`, first example per page open by default), plus a minimal regression test (`frontend/tests/pages/TopicPage.test.tsx`) driving real routing through ChapterPage → TopicPage → QuestionPage. Explicitly scoped to exclude self-serve access changes, Revision/Review-incorrect features, teacher dashboard, GenAI/agents, and a full E2E framework.

**Content expansion — Exponents and Powers + 6 chapters to 420 total (2026-09-02, complete, commit `112ace7`).** New chapter Exponents and Powers added; all 7 chapters normalized to 60 questions each (420 total, up from 253). Content-only, via the existing Stage 10 pipeline.

**Content import — Squares and Cubes test questions (2026-08-19, complete, commit `a071335`, live in production).** Squares and Cubes expanded 40 → 52 questions via the existing Stage 10 pipeline. Content-only, no engineering changes. See `Development-Journal.md`'s 2026-08-19 entry.

**M2.4 — Content Activation Pilot: Linear Equations (2026-08-17, complete, commit `2e0205d`, live in production).** First real-content activation of the M2.1–M2.3 evaluators (which had shipped with zero production questions using them): Linear Equations converted to 3 `single_choice`, 2 `multi_choice`, 28 `numeric`, 11 remaining `short_text` (44 questions unchanged, 14/16/14 difficulty split unchanged). Content-only — no evaluator, schema, coaching, session-engine, or frontend source file touched. 267/267 backend, 88/88 Stage 10 pipeline, 134/134 frontend. See `Development-Journal.md`'s 2026-08-17 (M2.4) entry.

**M2.1–M2.3 — Question Response Semantics Foundation, Single Choice, Multi Choice (2026-08-17, complete, commits `e41670a`/`87e8414`/`e120a1d`).** Built the schema→evaluator→pipeline→UI chain for question-type diversity: additive `questionType`/`responseSpecification` on `Question` (default `short_text`); an `Evaluator` protocol + registry dispatch in `evaluation_service.py`; `numeric_tolerance_v1`/`single_choice_v1`/`multi_choice_v1` evaluators; Stage 10 structural validation; frontend `QuestionResponseInput` dispatch + `SingleChoiceInput`/`MultiChoiceInput`. A capability slice, not an activation slice — see M2.4 above. Architecture recorded in [ADR-008](ADR/ADR-008-question-response-evaluation-architecture.md). Full design record: `Question-Response-Semantics-Design-Proposal.md`.

**Slice A1 — Structured Learning Content Foundation (2026-08-17, complete, commit `6285263`, live in production).** Additive `Topic.concepts`/`.workedExamples` (structured `Concept`/`LearningObjective`/`WorkedExample`) and `Question.objectiveIds`, alongside unchanged legacy fields. A Square and A Cube migrated as the sole pilot chapter. Zero frontend/evaluation/coaching/Learning Session Engine changes. See `Phase-1-Handoff.md` §8.
...
Product Strategy Documentation Alignment (2026-09-02) — `Product-Vision.md` and `LearningExperienceArchitecture.md` fully rewritten, `Roadmap.md`/`ProductArchitecture.md`/`README.md`/`Phase-1-Handoff.md`/this file substantively edited, to a newly agreed North Star: one common Learner→Content→Attempt→Evidence→Intervention→Assessment→Result engine (not separate anonymous/class engines), assessment framed as not inherently teacher-only, mastery/fluency/transfer/readiness and a cognitive-demand ("twist"/transfer) taxonomy documented as explicit future direction (not implemented), a deterministic-first GenAI strategy with 5 named future use cases, and an explicit 9-item non-goals list. Conflicting old framing (assessment as a permanently teacher-facing surface; a stale "no quiz construct exists" claim in `ProductArchitecture.md`) was corrected in place rather than left standing. Documentation-only — no application/product code, schema, API, UI, or content file changed in this pass.
...
M2 Documentation & Architecture Reconciliation (2026-08-19) — `Backlog.md`, `Roadmap.md`, this file, `Development-Journal.md`, `Phase-1-Handoff.md`, `Release-Notes.md`, and `Question-Response-Semantics-Design-Proposal.md` updated to match the verified state after M2.1–M2.4 and the Squares & Cubes content import (267/267 backend, 134/134 frontend, 88/88 Stage 10 pipeline; Linear Equations 3 single_choice/2 multi_choice/28 numeric/11 short_text; Squares & Cubes 52 questions); [ADR-008](ADR/ADR-008-question-response-evaluation-architecture.md) written for the evaluator-registry architecture built across M2.1–M2.4. Documentation-only — no code, schema, evaluator, pipeline, UI, test, or content file changed.
...
**440/440 backend tests passing, 250/250 frontend tests passing, `tsc -b`/`oxlint` clean** (fresh run, 2026-09-16, against the working tree with Self-Serve continuation + M3 + D1 + telemetry/mastery uncommitted). Stage 10 pipeline suite: **99/100 passing** via `node --test tests/*.test.js` (this suite uses Node's built-in test runner, not vitest — running it via vitest reports false "no test suite found" failures). The 1 failure (`pilotExport.test.js`'s "produces exactly 52 questions" assertion) is **pre-existing and unrelated to this release** — it hardcodes Squares & Cubes' old 52-question count from before the already-committed 2026-09-02 content expansion (`112ace7`) normalized every chapter to 60 questions; the test file itself is untouched by this release's diff. Not fixed here (out of this release's scope); flagged for its own follow-up.

**304/304 backend tests passing, 152/152 frontend tests passing** (re-run fresh and confirmed 2026-09-02, alongside the content expansion and TopicPage UX slice; Stage 10 pipeline suite not re-run this pass — last known count 88/88, 2026-08-19). Growth since the 267/134 counts below reflects the content-expansion and UX-slice commits (`112ace7`/`44b98b7`) plus other in-progress work visible in the working tree as of 2026-09-02 (see `git status`) not yet folded into a dated entry here.

**267/267 backend tests passing, 134/134 frontend tests passing, 88/88 Stage 10 pipeline tests passing** (as of M2.4 and the Squares & Cubes content import — 2026-08-19; `tsc -b`/`oxlint` clean). 205 → 267 backend and 112 → 134 frontend across Slice A1 and M2.1–M2.4 (Slice A1: 205 → 219 backend, +14; M2.1–M2.4: 219 → 267 backend, +48, and 112 → 134 frontend, +22, across the evaluator registry, three new evaluators, and two new frontend response components — see `Development-Journal.md`'s 2026-08-17 entries for the per-slice breakdown). The Curriculum Expansion Milestone and Release 0.1.2 below left both counts at 205/112 — the paragraph immediately following describes that now-superseded state, preserved as history.

Eight architecture decisions are now on record, all implemented and verified against shipped code — [ADR-008](ADR/ADR-008-question-response-evaluation-architecture.md) (question/response evaluation architecture — the `Evaluator` protocol, registry dispatch, and schema→evaluator→pipeline→UI capability chain built across M2.1–M2.4) joins the seven below.
...
✓ Structured Learning Content Foundation (Slice A1 — complete 2026-08-17: additive `Topic.concepts`/`.workedExamples`, `Question.objectiveIds`, A Square and A Cube migrated as pilot — see above)

✓ Question Response Semantics Foundation, Single Choice, Multi Choice (M2.1–M2.3 — complete 2026-08-17: `questionType`/`responseSpecification`, evaluator registry, `numeric`/`single_choice`/`multi_choice` evaluators, frontend response dispatch — see above and [ADR-008](ADR/ADR-008-question-response-evaluation-architecture.md))

✓ Content Activation Pilot: Linear Equations (M2.4 — complete 2026-08-17: real content converted to the M2.1–M2.3 types for the first time — see above)
...
**Release preparation — Self-Serve continuation, M3, D1, telemetry/mastery — staged 2026-09-16, commit SHA(s) not yet assigned at the time this entry was written.** See "Engineering Milestone" above for full scope. This entry should be updated with the real commit SHA(s) once committed, per this file's own convention.

**Self-Serve Learning Loop V1, Slices 1–6b + hardening — complete, committed as `2335c8a`…`8187e98` (9 commits), 2026-09-03 to 2026-09-09, production-live status not verified in this documentation pass.** See "Engineering Milestone" above.

**Content expansion + chapter lesson-page UX slice — complete, committed as `112ace7`/`44b98b7`, 2026-09-02 (production-live status not re-verified in this documentation pass — see `Phase-1-Handoff.md` §18).** 7 chapters, 420 questions total (60 each); TopicPage gained a top-shortcut CTA and progressive-disclosure worked examples. See "Engineering Milestone" above.

**Content import — Squares and Cubes test questions — complete, committed as `a071335`, deployed and live in production, 2026-08-19.** Squares and Cubes expanded 40 → 52 questions. See "Engineering Milestone" above.

**M2.4 — Content Activation Pilot — complete, committed as `2e0205d`, deployed and live in production, 2026-08-17.** Linear Equations converted to 3 single_choice/2 multi_choice/28 numeric/11 short_text (44 questions, count and difficulty split unchanged). See "Engineering Milestone" above and `Question-Response-Semantics-Design-Proposal.md`'s pilot-chapter reconciliation note.

**M2.1–M2.3 — Question Response Semantics — complete, committed as `e41670a`/`87e8414`/`e120a1d`, deployed and live in production, 2026-08-17.** `questionType`/`responseSpecification`, evaluator registry, three new evaluators, frontend response dispatch. See "Engineering Milestone" above and [ADR-008](ADR/ADR-008-question-response-evaluation-architecture.md).

**Slice A1 — Structured Learning Content Foundation — complete, committed as `6285263`, deployed and live in production, 2026-08-17.** Additive `Topic.concepts`/`.workedExamples`, A Square and A Cube migrated as pilot. See `Phase-1-Handoff.md` §8.
...
**Updated 2026-09-16 — M3 (`multi_part`, item 3 in the superseded list two paragraphs below) is now done**, released alongside Self-Serve continuation, D1 (concept-level performance, not previously named in this file at all), and telemetry/mastery — see "Engineering Milestone" above. Attempt telemetry enrichment (item 4 below) is also now done, as part of the same release. The two-engine split (item 2, "Blocked on a product-owner decision") remains not started and not authorized.

**Updated 2026-09-02 — the paragraph below is stale and partially wrong; corrected here.** Slice A2 (TopicPage structured-content cutover) and A2b (migrating the remaining Topic-bearing chapters) are **both done** — commits `51a05fe` and `d7890cc`, discovered in `git log` during this update after this file's own prior claim that neither had started turned out to be stale (see `Phase-1-Handoff.md` §8's corrected note for the full account and `Structured-Learning-Content-Design-Proposal.md`'s status line, which is also stale and not yet corrected). Slice A3 (removing the now-redundant legacy Topic fields) has not been done. Per `Roadmap.md`'s North Star Capability Model (the current authoritative next-priority source, superseding the paragraph below and `Backlog.md`'s old "Recommended Next"): the single highest-leverage near-term item is exposing the already-authored misconception fields (`commonWrongAnswer`/`why`/`remediationHint`) through the pipeline + API. The single largest gap is the two-engine (anonymous vs. class-joined) split, explicitly flagged "blocked on a product-owner decision," not authorized to start. Independent, non-blocking track: operate Shadow Mode and gather data (toward scoping confidence-gated live evaluation, not yet numbered).

*Superseded paragraph, kept for history:* "The 2026-07-28 pending decision between 'Structured Learning Content / Topic schema improvements' and 'Answer Evaluation v2 / mathematical answer semantics' was not an either/or — both were pursued and both shipped: Structured Learning Content as Slice A1 (2026-08-17, `6285263`), Answer Evaluation v2 as M2.1–M2.4 (2026-08-17, `e41670a`/`87e8414`/`e120a1d`/`2e0205d`). Current next objective, per the agreed priority sequence (see `Backlog.md`'s 'Recommended Next'): this Documentation & Architecture Reconciliation, then Slice A2 ..., then A2b .... None of this is started ..." — both slices have since shipped; see the corrected paragraph above.

## Recommended Next Milestone (Documentation)

This reconciliation (2026-08-19), triggered by M2.1–M2.4 and the Squares & Cubes content import leaving `Backlog.md`/`Roadmap.md`/`PROJECT_STATUS.md`/`Development-Journal.md`/`Phase-1-Handoff.md`/`Release-Notes.md` stale and no ADR covering the evaluator-registry architecture. The next documentation milestone should be triggered by the next major engineering milestone, per `AI-Builder-OS/DOCUMENTATION_STANDARDS.md`'s audit cadence, not on a fixed schedule — **this 2026-09-02 Product Strategy Documentation Alignment is that next trigger**, itself surfacing that A2/A2b had shipped undocumented; see the corrected "Next Engineering Objective" above.
...
**Stale since 2026-08-19 — corrected 2026-09-16.** This section's prior claim of "none" was wrong for everything that happened after `a071335`: the entire 2026-09-02 Product Strategy Documentation Alignment, all 9 Self-Serve commits' worth of follow-on work, M3, and D1 all accumulated in the working tree without this section ever being updated. As of 2026-09-16, immediately before this release's commits: the four workstreams named in "Engineering Milestone" above (Self-Serve continuation, M3, D1, telemetry/mastery) are staged for commit as part of this release. Genuinely unrelated items that remain excluded from this and every prior release: the `.gitignore` Chess-related change, `chess_poll.png`, `chess_ui_dump.xml`. One correction from earlier in this same pass: the repo-root `app/data/runtime.db` was initially assumed to be an unrelated stray artifact by location alone — it is actually this backend's own real runtime database, written to the wrong path because the dev server used during this release's manual validation was launched with a working directory that didn't match its `DATA_DIR=app/data` relative-path setting (a local launch-config quirk, not an application bug — see the 2026-09-16 "Last Verified" entry below). It is still correctly excluded from any commit (a generated, gitignored local artifact either way), just not for the reason first assumed.

Run `git status` before starting new work, as always — this section drifting stale for a month is exactly the failure mode that check exists to catch.

---

## Last Verified

Self-Serve continuation + M3 + D1 + telemetry/mastery release ✔ (2026-09-16, commit candidate — not yet committed at the time this entry was written: 440/440 backend pytest, 250/250 frontend vitest, `tsc -b`/`oxlint` clean, 99/100 Stage 10 pipeline via `node --test` (1 pre-existing unrelated failure, see "Engineering Health"). **Live manual validation**, both dev servers running: anonymous flow — answered `le-q01` as a brand-new learner, confirmed `GET /auth/me` 401 → `POST /auth/learner/start` 200 sequencing, identity/session/Dashboard data all survived a full page reload. Solution-reveal fix confirmed live — 3 wrong answers with zero hints clicked correctly revealed "Reveal Solution". M3 confirmed live for all three questions: `le-q25` (LHS/RHS) and `le-q37` (smaller/larger number) both correct, `le-q37` retried with one part deliberately wrong → correctly marked overall incorrect, `le-q40` (length/breadth) correct; typing into only one part left Check Answer disabled; reload mid-multi-part-answer produced no crash or corrupted state, just a clean reset. `le-q25` also confirmed live through the tracked Session flow (`SessionQuestionPage`) including a hint reveal mid-question. TopicPage's "Start a Tracked Practice Session" CTA confirmed live from a direct (non-Dashboard) topic visit, correctly establishing a fresh identity before routing into the Session engine. Mastery behavior confirmed live end-to-end: a hint-assisted correct answer reset `currentStreak` to 0 (`GET /performance/me`), and three subsequent hint-free correct answers produced `currentStreak: 3, mastered: true`. D1 confirmed live through the full Dashboard → service → API → card path: per-concept accuracy (`11/11`, `8/8`, "Not yet attempted" for the untouched concept) rendered correctly, and `le-q30` (the one real cross-concept question) correctly posted its full correct outcome to both of its concepts independently, not split or averaged. `submitted_option_id` persistence confirmed via direct attempts-table inspection. **One environment-only finding, not a code defect**: the dev server launched for this validation pass wrote `runtime.db` to the repo root instead of `backend/app/data/` because of a working-directory mismatch between the root-level `.claude/launch.json` config and the backend's relative `DATA_DIR` setting — cross-checked directly against the database in both locations to confirm every write actually happened correctly, just at an unexpected path; does not affect the normal `cd backend && uvicorn ...` workflow `Developer-Runbook.md` documents. Full milestone-provenance verification: `REL-2026-09-16-selfserve-m3-d1-telemetry-v2` — 54/54 expected files confirmed changed, 0 unresolved holds, 4 out-of-scope items correctly excluded (`.gitignore`, `app/data/runtime.db`, `chess_poll.png`, `chess_ui_dump.xml`), 8 pre-existing stale documentation files correctly left untouched by this release's scope.)

Content expansion + UX slice + documentation reset ✔ (2026-09-02: `backend/app/data/{chapters,questions,topics}.json` read directly — 7 chapters, 420 questions, 6 Topics, confirmed; backend `pytest` 304/304 and frontend `vitest run` 152/152 re-run fresh and confirmed passing; production-live status of `112ace7`/`44b98b7` **not** re-checked in this pass, per `Phase-1-Handoff.md` §18. Documentation reset (`Product-Vision.md`, `LearningExperienceArchitecture.md`, `Roadmap.md`, `ProductArchitecture.md`, `README.md`, `Phase-1-Handoff.md`, this file) is documentation-only — verified by re-reading every edited file, not by any test suite.)

M2.4 + Squares & Cubes content import ✔ (live in production, 2026-08-19: 267/267 backend pytest, 88/88 Stage 10 pipeline, 134/134 vitest, `tsc -b`/`oxlint` clean; local `main`/`origin/main` synchronized at `a071335`; Linear Equations verified at 3 single_choice/2 multi_choice/28 numeric/11 short_text (44 questions, 14/16/14 difficulty split unchanged); Squares and Cubes verified at 52 questions; anonymous and Learning Session flows both live-verified rendering the new radio/checkbox question types with no answer leakage — see `Development-Journal.md`'s 2026-08-17 (M2.4) entry for the full verification record)
