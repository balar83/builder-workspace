...
See `Backlog.md`'s "Recommended Next": write ADR-006 (planning/selection, C1) and ADR-007 (runtime, C2), now that the full Learning Session Engine is implemented, tested, and live-verified; P1's Question-page UX fixes (independent, addresses the measured 44-item progress-dot crowding); operate Shadow Mode and gather data (toward scoping confidence-gated live evaluation, not yet numbered); export Data Handling's already-authored 42 questions (nearest next content step). A frontend for the Learning Session Engine (C1+C2) is a related, not-yet-scoped need — no UI exists for `POST /sessions` or any of its sibling routes. See `Roadmap.md`'s "Scalable Assessment System" section for the fuller Milestone A–F sequencing, and `ADR/ADR-001-evaluation-coaching-separation.md` / `ADR/ADR-002-shadow-mode-execution-and-logging.md` / `ADR/ADR-003-content-authoring-and-export-pipeline.md` / `ADR/ADR-004-student-teacher-identity.md` / `ADR/ADR-005-server-side-attempt-history.md` for the seams any future engineering builds on.
...
Everything through Milestone C1 is committed as of 2026-07-28. **Milestone C2 (Learning Session Engine, stateful runtime) is implemented, tested, and live-verified, but not yet committed** — new `app/services/{session_store,session_builder,runtime_session_manager}.py`, `app/api/routes/sessions.py`, matching test files (`test_session_store.py`, `test_session_builder.py`, `test_runtime_session_manager.py`, `test_sessions_api.py`); modified `app/schemas/session.py` (C2 additions), `app/services/attempt_service.py` (`attempts.db` → `runtime.db` rename), `app/services/content_repository.py` (`get_question_content`), `app/api/router.py` (sessions router wired in), `backend/.gitignore` (`runtime.db`), `test_content_repository.py`, and this documentation pass.
...
Backend ✔ (198/198 pytest — 151 pre-Milestone-C2, 94 pre-Milestone-C1, 79 pre-Milestone-B, 65 pre-Milestone-A, 60 pre-Feature-018, up from 47 pre-Feature-015 — see below)

Frontend ✔ (49/49 vitest, `tsc -b`, `oxlint`, `vite build` all clean — unchanged by Milestone B, C1, or C2, no frontend work in any of the three slices)
