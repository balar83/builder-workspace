...
**Release 0.1.1 — Curriculum Expansion, complete 2026-08-07.** Data Handling (reviewed, approved, exported: 5 → 42 questions, 1 Topic) and Understanding Quadrilaterals (authored from scratch, exported: 5 → 40 questions, 1 Topic) both live via the Stage 10 pipeline. Practical Geometry remains the only chapter still on its original 5 hand-seeded placeholder questions with no Topic.

**Release 0.2, first slice implemented 2026-07-27, not yet complete.** Content Pipeline & Topic Delivery (Features 018–021) shipped Linear Equations end-to-end (Learn + Worked Examples stages, per `LearningExperienceArchitecture.md`), now joined by Data Handling and Understanding Quadrilaterals via Release 0.1.1. Release 0.2's Understand stage (comprehension check) is still not built for any chapter. Release 0.1 ("It Remembers You") shipped the same day as Features 018–021, a separate non-blocking track. Feature 015 (Shadow Mode) shipped 2026-07-23 and remains operable but hasn't yet accumulated a meaningful sample of real evaluations — see [ADR-002](ADR/ADR-002-shadow-mode-execution-and-logging.md).

**Version 1.0 readiness track (2026-07-29, now complete through RC1).** `Release-Plan-v1.0.md` defines what Version 1.0 is — the first stable, end-to-end coaching experience for daily student use, not a feature-complete product. Milestone F1 (the Session Frontend) is now fully built and live-verified; Milestone RC1 assessed the application ready for a `v1.0.0-rc1` tag. Cutting the actual tag and deploying it are the user's own decisions, not yet done as of this entry.

---

## Next Engineering Objective

**Tag and deploy `v1.0.0-rc1`** (a user decision — see `Deployment-Guide.md`'s release checklist), then let it run under real daily use before deciding on a final `v1.0.0`. No further Session Frontend engineering is queued; do not begin Assessment Engine or Version 1.1 work without explicit instruction, per this milestone's own stated boundary. Independent, non-blocking tracks available in parallel: P1's Question-page UX fixes (addresses the measured 44-item progress-dot crowding, now also relevant to Data Handling's 42 and Understanding Quadrilaterals' 40); operate Shadow Mode and gather data (toward scoping confidence-gated live evaluation, not yet numbered); Practical Geometry remains the one chapter with no content-source authoring trail and no Topic. See `Roadmap.md`'s "Scalable Assessment System" section for the fuller Milestone A–F sequencing, and `docs/ADR/` for the seven accepted ADRs any future engineering builds on.
...
None as of this documentation pass — Sprint C's implementation (Session Completion, Resume, UX polish), the Runbook/Deployment-Guide corrections found during clean-clone validation, and this file's own updates are committed together. See `git log` for the exact hash.
